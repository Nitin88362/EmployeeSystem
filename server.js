...
// Database pool with SSL support (for Aiven/PlanetScale/cloud MySQL)
const dbConfig = {
...
// Enable SSL for managed MySQL (Aiven, PlanetScale, AWS RDS, etc.)
...
async function bootstrap() {
  ...
  const tables = [
    ...
    `CREATE TABLE IF NOT EXISTS employees (
      ...
      join_date DATE, dob DATE,
      gender VARCHAR(20), address TEXT,
    ...
    `CREATE TABLE IF NOT EXISTS attendance (
      ...
      in_status VARCHAR(20), out_status VARCHAR(20),
      ...
      manual_by VARCHAR(100), manual_reason TEXT,
      created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
      UNIQUE KEY uk_emp_date (employee_id, date),
      FOREIGN KEY (employee_id) REFERENCES employees(id) ON DELETE CASCADE,
      INDEX idx_date (date), INDEX idx_emp (employee_id)
    )`,
    `CREATE TABLE IF NOT EXISTS leaves (
      ...
      FOREIGN KEY (employee_id) REFERENCES employees(id) ON DELETE CASCADE,
      INDEX idx_status (status), INDEX idx_emp (employee_id)
    )`,
    ...
    `CREATE TABLE IF NOT EXISTS payroll (
      ...
      working_days INT, present_days INT,
      paid_leave_days DECIMAL(5,1), lop_days DECIMAL(5,1),
      total_late_min INT, total_ot_min INT,
  ...
  await pool.execute(
    'INSERT IGNORE INTO admin (username, password_hash, name, email) VALUES (?, ?, ?, ?)',
  ...
  const leaveTypes = [
    ...
    ['earned', 'Earned Leave', 15, 'green', 1, 3],
    ['lop', 'Loss of Pay', 0, 'gray', 0, 4],
  ];
  for (const lt of leaveTypes) {
    await pool.execute('INSERT IGNORE INTO leave_types (id, name, annual_quota, color, is_paid, sort_order) VALUES (?, ?, ?, ?, ?, ?)', lt);
  }
  ...
  for (const d of depts) {
    await pool.execute('INSERT IGNORE INTO departments (id, name, head) VALUES (?, ?, ?)', d);
  }
  
  const yr = new Date().getFullYear();
  const holidays = [
    ['hol_rd', `${yr}-01-26`, 'Republic Day', 'national'],
    ['hol_id', `${yr}-08-15`, 'Independence Day', 'national'],
    ['hol_gj', `${yr}-10-02`, 'Gandhi Jayanti', 'national'],
    ['hol_diw', `${yr}-11-01`, 'Diwali', 'festival'],
    ['hol_holi', `${yr}-03-25`, 'Holi', 'festival'],
  ];
  for (const h of holidays) {
    await pool.execute('INSERT IGNORE INTO holidays (id, date, name, type) VALUES (?, ?, ?, ?)', h);
  }
  ...
  await pool.execute(
    ...
    ['emp_demo', 'EDT001', 'Demo Employee', 'demo@editone.in', demoHash, '9999999999',
     ...
     25000, 10000, 5000, '{"casual":12,"sick":12,"earned":15,"lop":0}', 'active']
  );
  
  console.log('✓ Default data seeded');
  console.log(`✓ Admin: ${adminUser} / ${adminPass}`);
  console.log('✓ Demo: demo@editone.in / demo123');
}
...
(async () => {
  try {
    ...
    // Auto-bootstrap on first run (creates tables if missing)
    await bootstrap();
    ...
    console.error('\nCheck .env file:');
    console.error('  - DB_HOST, DB_USER, DB_PASSWORD, DB_NAME correct hain?');
    console.error('  - Cloud MySQL ke liye DB_SSL=true set karein\n');
    process.exit(1);
  }
})();
